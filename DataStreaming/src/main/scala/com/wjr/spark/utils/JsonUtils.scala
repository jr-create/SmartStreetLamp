...
object JsonUtils {
    ...
    def getObjectValue(data: Any): Array[AnyRef] = {
        JsonUtils.getFields[DeviceCountInfo].toBuffer.map(
            FieldAndValue.getFieldValueByName(_, data)
        ).reverse.toArray
    }
    ...
    private[spark] def ObjectToJson(data: Any): String = {
        try {
            ...
            println("发送到Kafka的数据：" + str) // TODO: 将CountCase样例类转为json
            str
        } catch {
            case e: Exception =>
